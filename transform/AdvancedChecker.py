...
class AdvancedChecker(Input):
    ...
    def check(self, tsvPath=None, corpusPath=None, corpus=None, fact=None):
        ...
        if len(truthValues) > 0:
            ...
            for i in range(min(len(truthValues), len(values))):
                if truthValues[i] == values[i]:
                    correct += 1
            print("Correctly Found: {} out of {} ({}%)".format(
                correct, len(values), (correct / len(values))*100))
                ...
                with open(corpusPath, "r", encoding="utf-8") as corpus:
                    for article in corpus:
                        if obj1 in article or obj2 in article:
                            value, certain = AdvancedChecker.checkFactOnArticle(
                                obj1, rest, obj2, article)
                            if certain:
    ...
    def CheckFactWithTriples(self, triple, corpusPath, corpus=None):
        ...
        for article in corpus:
            ...
            if subj in article or obj in article:
                # self.checkFactOnSentences(subj, verb, obj, article)
                value, certain = AdvancedChecker.checkFactOnArticle(
                    subj, verb, obj, article)
                if certain:
                    # return value if we are certain
                    return value
        if corpus == None:
